...
Every file is dependency-free standard library. Copy the ones you need.

```
kit/webhook_rail.py   a payment rail that cannot silently lose money
kit/gates.py          gates that fail OPEN on "couldn't measure", CLOSED on "measured bad"
kit/staleness.py      catch loops that run forever and produce nothing
kit/attention.py      budget arbitration that proves nothing starves
kit/delivery.py       idempotent fulfilment that never drops a paid order
tests/test_kit.py     29 tests — the regression suite for all five incidents
```
